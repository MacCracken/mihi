...
| `mihi_cpu_model`  | `/proc/cpuinfo` first `model name` value    | man 5 proc; kernel `fs/proc/cpuinfo.c` → arch `show_cpuinfo()` (e.g. `arch/x86/kernel/cpu/proc.c`)                                        | Line-anchored search for `"\nmodel name"` pins us to CPU 0's block — first-block-only honours "one source per fact" on big.LITTLE. Caller supplies 8 kB scratch, which is *CPU 0's block fits*, not *the file fits*: /proc/cpuinfo is one block per logical CPU and measures 26 kB on a 16-thread host. The only read in mihi using `MIHI_IO_PREFIX`. **Three targets, three sources, no fallback between them** — AGNOS → CPUID brand; **aarch64 Linux → `/sys/firmware/devicetree/base/cpus/cpu@0/compatible`** (arm64's `c_show()` emits no `model name` line at all, audit D-1, closed at 1.2.4 and verified on a Raspberry Pi 4: `arm,cortex-a72`); x86 Linux → this row. ⚠ ACPI-booted arm64 has no device tree → still null. The two Linux arms return different flavours of string (marketing brand vs DT compatible tuple), both raw. |
...
## Slice B2 — aarch64 Linux CPU model (1.2.4)

| Probe | Source | Authority | Notes |
| --- | --- | --- | --- |
| `mihi_cpu_model` (aarch64 Linux) | `/sys/firmware/devicetree/base/cpus/cpu@0/compatible` | Devicetree Specification, `compatible` property; Linux `drivers/of/base.c` exposing the DT via `/sys/firmware/devicetree/base` (with `/proc/device-tree` as the kernel's compatibility symlink) | A DT property blob **is** the value and carries its own NUL — 15 bytes for `"arm,cortex-a72\0"`, no trailing newline — so it is read `MIHI_IO_WHOLE` and parsed by `mihi_parse_dt_string`, not by the `/proc` line parsers. `compatible` may be a NUL-separated list, most specific first; the first entry is the answer per DT convention. Anchored at `cpu@0` for the same reason the x86 path anchors at the first `/proc/cpuinfo` block. Measured on agnosarm (RPi 4, kernel 6.8.0-1053-raspi). |
...
| Probe                    | Source                                                  | Authority                                                                                                                 | Notes |
| ------------------------ | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----- |
| `mihi_gpu_count`         | `ai-hwaccel::registry_detect_no_exec()` profile vec     | ai-hwaccel 2.3.22 `src/registry.cyr` (no-exec orchestrator) + per-backend sysfs paths                                       | Excludes the synthetic CPU profile. Lazy: first call across any `mihi_gpu_*` runs detection. |
| `mihi_gpu_name(idx)`     | `ai-hwaccel::profile_device_name(p)`                    | ai-hwaccel 2.3.22 `src/detect/{rocm,tpu,gaudi,neuron,intel,amd_xdna,edge,cloud_asic}.cyr` (each backend's name setter)        | ROCm prefers `/sys/class/drm/cardN/device/product_name`, falls back to a synthesized `AMD Radeon (PCI vendor:device)` string. Other safe backends hardcode vendor strings. Returns 0 on out-of-range idx. |
...
(9 exec / 9 no-exec split, **re-measured against ai-hwaccel 2.3.22** at
the 1.2.6 pin rather than re-stamped: `AIHW_BACKEND_COUNT` is 18 and
`backend_uses_exec` returns 1 for exactly nine — `BACKEND_WINDOWS`
